...
const Footer = () => {
  ...
  return (
    <footer className={`${styles.footer} bg-[#010103] border-t border-amber-500/30 relative z-[100] py-8 md:py-12`}>
      {/* Top Border Glow Line */}
      <div className="absolute top-0 left-0 w-full h-[1px] bg-amber-400/40 shadow-[0_0_10px_#f5ae37]" />
      ...
      <div className={`${styles.container} max-w-7xl mx-auto px-6`}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          >
            <div className="flex gap-4 mb-6 items-center relative">
              <div className="absolute -left-2 w-24 h-24 bg-amber-500/5 blur-[30px] rounded-full" />
              ...
              <img
                src="/k1000-small.png"
                alt="K-1000 Logo"
                className="h-[45px] object-contain brightness-125 drop-shadow-[0_0_8px_rgba(245, 174, 55,0.5)] animate-pulse relative z-10"
              />
            </div>
            <p className="text-white text-[13px] leading-relaxed max-w-[280px] font-normal">
              K-1000 is KIIT&apos;s premier <span className="text-amber-400 font-bold">R&D initiative</span>, empowering 1000
            ...
            <ul className="space-y-3">
              {quickLinks.map((link, index) => (
                <li key={index}>
                  <Link
                    href={link.path}
                    className={`flex items-center gap-2 text-[16px] transition-all hover:text-amber-400 group ${
                      pathname === link.path ? "text-amber-400" : "text-white"
                    } font-medium tracking-wide`}
                  >
                    <ChevronRight size={14} className={`${pathname === link.path ? 'text-amber-400' : 'text-amber-400/30'} group-hover:translate-x-1 transition-transform`} />
                    {link.title}
                  </Link>
                </li>
              ))}
            ...
            <ul className="space-y-3">
              {researchAreas.map((area, index) => (
                <li key={index}>
                  <Link
                    href={area.href}
                    className="flex items-center gap-2 text-white text-[16px] hover:text-amber-400 group transition-all font-medium tracking-wide"
                  >
                    <ChevronRight size={14} className="text-amber-400/30 group-hover:translate-x-1 transition-transform" />
                    {area.title}
                  </Link>
                </li>
              ))}
          ...
          >
            ...
            <div className="flex gap-5 items-center justify-start mb-6">
              <a href="https://www.linkedin.com/company/k1000-kiit" target="_blank" className="hover:scale-110 transition-transform group">
                <FaLinkedinIn className="text-white text-xl group-hover:text-amber-400 transition-all" />
              </a>
            ...
            <div className="space-y-4">
              <div className="flex gap-3 items-start text-white text-[16px] font-medium">
                ...
                <span className="leading-tight">KIIT University, Bhubaneswar,<br/><span className="text-white text-[10px] uppercase tracking-wider">Odisha 751024</span></span>
              </div>
              <div className="flex gap-3 items-start text-white text-[16px] font-medium">
                <Phone className="text-amber-400 shrink-0" size={16} />
                <span className="tracking-wide">Dr. Ajit Kumar Pasayat<br/><span className="text-amber-400 text-[11px]">+91 7008588187</span></span>
              </div>
              <div className="flex gap-3 items-start text-white text-[16px] font-medium">
                <Mail className="text-amber-400 shrink-0" size={16} />
                <span className="underline underline-offset-4 decoration-amber-400/30 hover:text-amber-400 transition-colors">k.1000@kiit.ac.in</span>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-12 pt-6 pb-2 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-[10px] text-white uppercase tracking-[0.3em] font-medium text-center md:text-left">
            © {new Date().getFullYear()} KIIT University.
          </p>
          <div className={`${conthrax} text-[7px] text-amber-500/60 tracking-[0.8em] font-black uppercase text-center md:text-right w-full md:w-auto`}>
