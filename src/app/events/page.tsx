...
const Events = () => {
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#020202] text-white selection:bg-amber-500/30 overflow-x-hidden cursor-default">
      <CubeBackground zIndex={0} disableLinesOnMobile />
      <SharedHeader />

      <motion.div
        className="fixed top-0 left-0 right-0 h-[2px] bg-amber-500 z-[100] origin-left"
        style={{ scaleX }}
      />
      ...
      <main className="relative z-10 max-w-[1600px] mx-auto pt-24 md:pt-32 pb-16 md:pb-24 px-4 md:px-10">
        ...
        <div className="mb-10 md:mb-20 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-3">
            ...
            >
              Event{" "}
              <span className="text-amber-400 drop-shadow-[0_0_15px_rgba(245, 174, 55,0.4)]">
                Registry
              </span>
            </h1>
          </div>
          <div className="hidden lg:block text-right">
            <p className={`${orbitron} text-[10px] text-white tracking-widest uppercase`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 lg:items-start">
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-28 z-30">
            <div
              ref={navRef}
              className="flex overflow-x-auto overscroll-x-contain bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[24px] lg:rounded-[32px] p-2 pr-5 lg:pr-2 gap-1 md:max-h-[680px] md:flex-col md:overflow-x-hidden md:overflow-y-auto md:[scrollbar-color:rgba(245, 174, 55,0.35)_transparent] md:[scrollbar-width:thin]"
            >
                    ...
                    <div className="flex flex-col gap-1 min-w-0">
                      <span
                        className={`${orbitron} text-[8px] tracking-widest font-black truncate ${
                          selectedEvent.id === event.id ? "text-amber-400" : "text-white"
                        }`}
                      >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 lg:p-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div className="space-y-2 min-w-0">
                      <div className="flex items-center gap-2">
                        <Zap size={12} className="text-amber-400 fill-amber-400 flex-shrink-0" />
                        <span
                          className={`${conthrax} text-[9px] sm:text-[10px] text-amber-400 uppercase font-black tracking-[0.25em] sm:tracking-[0.3em] truncate`}
                        >
                    ...
                    {selectedEvent.status === "UPCOMING" ? (
                      ...
                      >
                        <Link
                          ...
                          className={`${conthrax} self-start sm:self-auto flex-shrink-0 flex items-center gap-2 sm:gap-3 px-5 sm:px-7 lg:px-8 py-3 sm:py-4 bg-amber-500 text-black rounded-full font-black text-[9px] sm:text-[10px] uppercase tracking-widest shadow-[0_0_30px_rgba(245, 174, 55,0.4)] cursor-pointer whitespace-nowrap`}
                        >
                ...
                <div className="rounded-[28px] sm:rounded-[36px] lg:rounded-[40px] bg-white/[0.025] backdrop-blur-md border border-white/10 p-6 sm:p-8 lg:p-10 shadow-[0_0_30px_rgba(0,0,0,0.18)]">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                    ...
                    <div className="space-y-6">
                      ...
                      <p className="text-white text-base sm:text-lg md:text-xl leading-relaxed font-light italic border-l-2 border-amber-500/20 pl-5 whitespace-pre-line">
                    ...
                    {/* Critical Highlights */}
                    <div className="bg-amber-500/[0.015] border border-amber-500/10 rounded-[28px] sm:rounded-[36px] lg:rounded-[40px] p-6 sm:p-8 lg:p-10">
                      <h4
                        className={`${conthrax} text-[9px] sm:text-[10px] text-amber-400 uppercase tracking-[0.4em] sm:tracking-[0.5em] mb-6 lg:mb-8 font-black`}
                      >
                        Critical Highlights
                      </h4>
                      <div className="space-y-4 sm:space-y-6">
                        {selectedEvent.highlights.map((point, i) => (
                          <div key={i} className="flex gap-3 sm:gap-4 group">
                            <div className="w-1.5 h-1.5 rounded-full bg-amber-400 mt-[5px] shrink-0 shadow-[0_0_10px_#f5ae37] group-hover:scale-150 transition-transform" />
                            <p className="text-white text-sm sm:text-sm md:text-base leading-snug font-light group-hover:text-white transition-colors whitespace-pre-line">
                              {point}
                            </p>
                          </div>
                        ))}
