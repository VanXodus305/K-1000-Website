...
export default function DepartmentsPage() {
  ...
  return (
    <div className="flex flex-col w-full bg-black text-white selection:bg-amber-500/30 overflow-x-hidden relative min-h-screen">
      <SharedHeader />
      ...
      <main className="relative z-10 flex flex-col items-center w-full px-4 sm:px-6 lg:px-8">
        ...
        <section className="w-full max-w-7xl pt-28 md:pt-40">
          <div className="relative w-full h-[280px] md:h-[450px] rounded-3xl md:rounded-[40px] overflow-hidden border border-amber-500/20 shadow-[0_0_50px_rgba(0,0,0,1)]">
            <img
            ...
            <div className="relative z-10 flex flex-col items-center justify-center text-center p-6 h-full">
              ...
              >
                <h1 className={`${conthrax} text-2xl xs:text-3xl sm:text-5xl md:text-7xl tracking-[0.2em] text-white uppercase font-black leading-tight`}>
                  KIIT <span className="text-amber-400 block sm:inline drop-shadow-[0_0_15px_#f5ae37]">DEPARTMENTS</span>
                </h1>
                <div className="flex items-center justify-center gap-2 md:gap-4 overflow-hidden">
                  <div className="h-[1px] w-8 md:w-20 bg-gradient-to-r from-transparent to-amber-500/50" />
                  <p className={`${conthrax} text-amber-400/70 tracking-[0.2em] md:tracking-[0.5em] text-[7px] md:text-xs uppercase font-bold whitespace-nowrap`}>
                    Academic Infrastructure Protocol
                  </p>
                  <div className="h-[1px] w-8 md:w-20 bg-gradient-to-l from-transparent to-amber-500/50" />
                </div>
              </motion.div>
            </div>
          </div>
        ...
        <section className="w-full max-w-7xl py-12 md:py-24 space-y-16 md:space-y-32">
          {categories.map((category, index) => (
            ...
            >
              ...
              <div className="absolute top-0 -left-6 md:-left-12 w-[1px] h-full bg-gradient-to-b from-amber-400/40 via-amber-400/5 to-transparent hidden md:block" />

              {/* Category Header */}
              <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 mb-8 md:mb-12">
                <div className="w-14 h-14 md:w-20 md:h-20 flex items-center justify-center text-2xl md:text-4xl text-amber-400 bg-amber-500/5 backdrop-blur-xl rounded-2xl border border-amber-500/20 shadow-[0_0_20px_rgba(245, 174, 55,0.1)]">
                  {category.icon}
                </div>
                <div className="space-y-1">
                  <span className={`${conthrax} text-[9px] md:text-[11px] text-amber-500/40 tracking-[0.4em] uppercase font-black`}>
                    {category.id}
                  </span>
                  <h3 className={`${conthrax} text-xl md:text-4xl text-white tracking-widest uppercase font-black group-hover:text-amber-400 transition-colors duration-500`}>
                    {category.title}
                  </h3>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                {category.schools.map((school, i) => (
                  <motion.div
                    ...
                    className="p-4 md:p-6 rounded-2xl bg-white/[0.03] backdrop-blur-md border border-white/5 flex items-center group/item transition-all duration-300 hover:border-amber-500/30 hover:bg-amber-500/[0.02]"
                  >
                    <div className="flex items-center gap-4">
                      <div className="shrink-0 w-1.5 h-1.5 rounded-full bg-amber-500/30 group-hover/item:bg-amber-400 group-hover/item:shadow-[0_0_8px_#f5ae37] transition-all" />
                      <span className={`${conthrax} text-[10px] md:text-[12px] text-white group-hover/item:text-white transition-colors font-bold tracking-wider uppercase leading-snug`}>
                        {school}
                      </span>
                    </div>
                  </motion.div>
